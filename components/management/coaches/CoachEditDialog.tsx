...
export default function CoachEditDialog({ coach }: { coach: CoachListItem }) {
  ...
  const { formData, handleChange, handleFieldChange, errors, handleSubmit, loading } =
    useFormManager<CoachFormData>({
      ...
      onSubmit: async (data, reset) => {
        const result = await updateCoach(coach.id, coach.profile_id, data);
  ...
  const infoForm = (
    <div className="flex flex-col gap-1 mt-1">
      <Input name="full_name" label={t("fields.fullName")} placeholder={t("placeholders.fullName")}
        value={formData.full_name} onChange={handleChange} error={errors.full_name} required />
      <Input name="phone" label={t("fields.phone")} placeholder={t("placeholders.phone")}
        value={formData.phone ?? ""} onChange={handleChange} error={errors.phone} type="tel" />
      <Textarea name="bio" label={t("fields.bio")} placeholder={t("placeholders.bio")}
      ...
      <div className="flex flex-col gap-1.5 mt-1">
        ...
        {specialties.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {specialties.map((s, i) => (
              <span key={i} className="inline-flex items-center gap-1 bg-[var(--accent-soft)] text-[var(--accent)] text-xs font-medium px-2 py-1 rounded-full">
                {s}
                <button type="button"
                  onClick={() => handleFieldChange({ name: "specialties", value: specialties.filter((_, j) => j !== i) })}
      ...
      <div className="flex gap-2 justify-end mt-4">
        ...
        <Button variant="accent" type="submit" isLoading={loading} onClick={handleSubmit}>
